...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    var options = {
      key: import.meta.env.VITE_PAYMENTKEY,
      key_secret: import.meta.env.VITE_PAYMENTSECRETKEY,
  ...
  useEffect(() =>{
    document.title="its Engineering - Cart"
},[]);
  return (
    <Layout >
      <div className="h-screen bg-gray-100 pt-5 lg:mb-[60%] " style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
          <div className="rounded-lg md:w-2/3 ">
            {cartItems.map((item, index) => {
              ...
              return (
                <div className="justify-between mb-6 rounded-lg border  drop-shadow-xl bg-white p-6  flex  justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
                  <img src={imageUrl} alt="product-image" className="w-20 h-20 rounded-lg sm:w-40 sm:h-auto" />
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between ">
                    <div className="mt-0 sm:mt-0">
                      <h2 className="lg:text-lg text-sm font-bold text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{title}</h2>
                      {/* <h2 className="text-sm  text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{description}</h2> */}
                      <p className="mt-1 text-xs font-semibold text-gray-700 lg:text-start text-end" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{price}</p>
                    </div>
                    <div onClick={() => deleteCart(item)} className="mt-4 flex justify-end sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
